 ...
 */
class DateTimeNormalizerPatch implements NormalizerInterface, DenormalizerInterface, CacheableSupportsMethodInterface
{
    ...
    public function __construct($defaultContext = [], \DateTimeZone $timezone = null)
    {
        ...
        if (!\is_array($defaultContext)) {
            @trigger_error(
                'Passing configuration options directly to the constructor is deprecated since Symfony 4.2, use the default context instead.',
                \E_USER_DEPRECATED
            );
    ...
    public function denormalize($data, $type, $format = null, array $context = [])
    {
        ...
        if (null !== $dateTimeFormat) {
            $object = \DateTime::class === $type ? \DateTime::createFromFormat(
                $dateTimeFormat,
                $data,
                $timezone
            ...
            $dateTimeErrors = \DateTime::class === $type ? \DateTime::getLastErrors(
        ...
        try {
            return \DateTime::class === $type ? new \DateTime($data, $timezone) : new \DateTimeImmutable(
                $data,
                $timezone
            );
        } catch (\Exception $e) {
            /**
             * #Fix bug invalid date format
             */
